...
此接口还需要包含Host、Date等公共请求头。关于公共请求头的更多信息，请参见[公共HTTP头定义](/cn.zh-CN/API 参考/公共HTTP头定义.md)。
...
|FileAlreadyExists|409|当请求的Header中携带`x-oss-forbid-overwrite=true`时，表示禁止覆盖同名文件。如果已存在同名文件，则返回该错误。|
