...
-   解决方法
    ...
    如果以上都没有问题，建议您部署tcpdump或者wireshark抓包，复现问题后分析下数据包。
...
-   错误原因1
    ...
    -   移除多余的httpclient依赖。引入OSS SDK时会自动引入httpclient依赖，如果是第三方库另外引入了httpclient，请参见[\#section\_u5d\_qgd\_kfb](#section_u5d_qgd_kfb)解决方案。
-   错误原因2
    ...
    -   参见[\#section\_u5d\_qgd\_kfb](#section_u5d_qgd_kfb)解决方案，移除冲突的httpclient版本。
