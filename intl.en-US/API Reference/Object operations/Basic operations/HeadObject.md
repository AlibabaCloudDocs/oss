...
By default, the HeadObject operation queries the metadata of the current version of an object. If the current version of the object is a delete marker, OSS returns 404 Not Found. If you specify a version ID in the request, OSS returns the metadata of the specified version of the object.
...
|If-None-Match|String|No|The information return condition. If the input ETag value does not match the ETag value of the object, OSS returns the metadata of the object and 200 OK. If the input ETag value matches the ETag value of the object, OSS returns 304 Not Modified. Default value: null |
...
-   The Last-Modified header is the later one of the last modified time of the symbolic link and the object to which the symbolic link points.
-   Other headers are the metadata of the symbolic link.

|Header|Type|Description|
|:-----|:---|:----------|
|x-oss-meta-\*|String|The user metadata prefixed with x-oss-meta-. If you specify headers prefixed with x-oss-meta- in the PutObject request when you upload an object, the user metadata of the object is included in the response.|
...
|x-oss-object-type|String|The type of the object. Valid values: -   Normal: The object is uploaded by using PutObject.
...
|x-oss-hash-crc64ecma|String|The 64-bit CRC value of the object. This value is calculated based on the ECMA-182 standard.If an object is created before OSS supports CRC64, this header may not be included in the response when you call HeadObject to query the metadata of the object. |
...
|x-oss-restore|String|The state of the object when you restore an object. This header is included in the response when the storage class of the bucket is Archive and a RestoreObject request is submitted. -   If the RestoreObject request is not submitted or expires, this header is excluded from the response.
-   If the RestoreObject request is submitted and the restoration is incomplete, the value of this header is ongoing-request="true".
...
|Content-Md5|String|-   This header is included in the response only when the object type is Normal. The value of this header is calculated as follows: 1. Calculate the MD5 hash of the message content except for the headers based on RFC 1864 to obtain a 128-bit digit. 2. Encode the digit by using Base64.
-   This header is not included in the response if the object type is Multipart or Appendable. |
|Last-Modified|String|The time when the object is last modified. The time is in GMT specified in HTTP/1.1.|
...
-   Sample requests for querying the metadata of an object in an unversioned bucket
...
-   Sample requests for querying the metadata of a specified object version in a versioned bucket
...
-   Sample requests for querying the metadata of the current object version in a versioned bucket
...
-   Sample requests for querying the metadata of an object for which the RestoreObject request is submitted but the restoration is incomplete
...
-   Sample requests for querying the metadata of an object for which the RestoreObject request is submitted and the restoration is complete
...
-   Sample requests for querying the metadata of an object that is encrypted by using SSE-OSS at the server side
...
-   Sample requests for querying the metadata of an object that is encrypted by using SSE-KMS at the server side
...
|Not Modified|304|-   The error message returned because the If-Modified-Since header is specified in the request, but the requested object has not been modified since the time specified in the request.
...
|Precondition Failed|412|-   The error message returned because the If-Unmodified-Since header is specified in the request, but the specified time is earlier than the modified time of the requested object.
