...
-   The sync command supports the --delete option. When you add this option in the command, if the destination of the synchronization is an OSS bucket, ossutil deletes all files that do not exist in the source from the destination bucket and retains only the synchronized files. If the destination of the synchronization is a local disk, you must also add the --backup-dir option in the command to migrate files that do not exist in the source but do exist in the destination local disk to the specified folder.

    **Warning:** To add the --delete option in the command, we recommend that you enable [versioning](/intl.en-US/Developer Guide/Data security/Versioning/Overview.md) for the destination bucket to prevent data from being accidentally deleted.
...
-   Examples
    ...
    The destination OSS bucket named examplebucket contains a folder named example, which contains two objects named a.txt and b.txt and a subfolder named C. In the root folder of the source local disk, a folder named example contains a file named d.txt. The following figure shows the structures of the destination bucket and the source local disk.
    ...
    -   Synchronize the example folder in the local disk to the destination OSS bucket.
        ...
        After the command is executed, the d.txt file is added to the example folder of the examplebucket bucket.
    ...
    -   Synchronize the example folder in the local disk to the destination OSS bucket and delete all objects in that folder of the examplebucket bucket that do not exist within the same folder in the local disk.
        ...
        After the command is executed, the example folder in the local disk is synchronized to the examplebucket bucket. The a.txt and b.txt files and the subfolder C within the example folder of the examplebucket bucket are deleted. Only the synchronized d.txt file is retained in the example folder of the examplebucket bucket.
...
-   Examples
    ...
    -   Synchronize the example folder of the examplebucket bucket to the local folder.
        ...
        After the command is executed, the a.txt and b.txt files and the subfolder C are added to the example folder in the local disk.
    ...
    -   Synchronize the example folder of the examplebucket bucket to the local folder, and migrate files that do not exist in example folder of the examplebucket bucket but do exist in the destination local folder to the backup folder of the examplebucket bucket.
        ...
        After the command is executed, the example folder of the examplebucket bucket is synchronized to the local disk, and the files in the example folder in the local disk are migrated to the backup folder of the examplebucket bucket. In this example, the a.txt and b.txt files and the subfolder C synchronized from OSS are retained in the example folder in the local disk. The d.txt file is added to the backup folder of the examplebucket bucket.
...
-   Examples
    ...
    -   Synchronize the example folder of the examplebucket1 bucket to the test folder of the same bucket.
        ...
        After the command is executed, all objects in the example folder are synchronized to the test folder.
    ...
    -   Synchronize the example folder of the examplebucket1 bucket to the example folder of the examplebucket2 bucket.
        ...
        After the command is executed, all objects in the example folder of the exmplebucket1 bucket are synchronized to the example folder of the examplebucket2 bucket.
    ...
    -   Synchronize all objects in examplebucket1 to examplebucket2, and delete objects that do not exist in examplebucket1 from examplebucket2.
        ...
        After the command is executed, the example and test folders of the examplebucket1 bucket are synchronized to the root folder of the examplebucket2 bucket. Other objects in the examplebucket2 bucket are deleted.
...
|--include|Specifies that objects whose names contain the specified value are matched. For example, if the value of this option is `*.jpg`, objects whose names end with `.jpg` are considered as matched.|
...
|--acl|Configures the ACL of an object. Valid values: -   default: ACL inherited from the bucket
-   private
-   public-read
-   public-read-write |
|--snapshot-path|If you specify the --snapshot-path option when you synchronize files, ossutil takes a snapshot of the synchronization and stores the snapshot information in a specified folder. When you specify this option in the command to synchronize the files again, ossutil reads the snapshot information from the specified folder and performs incremental synchronization. For more information, see [Generate snapshot information](/intl.en-US/Tools/ossutil/Common commands/cp/Upload objects.md).|
|--disable-crc64|Disables CRC64 verification in data transmission.By default, ossutil performs CRC64 verification in data transmission. |
|--maxupspeed|Specifies the maximum upload speed. Unit: Kbit/s.Default value: 0. The value of 0 indicates that the upload speed is not limited. |
...
|--proxy-user|Specifies the user name of the proxy server. This parameter is empty by default.|
