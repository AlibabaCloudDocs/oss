...
**说明：** 以下示例代码中的catch语法，请自行学习es6 promise、async/await。SDK的使用方式，请参见[安装](/intl.zh-CN/SDK 示例/Node.js/安装.md)。

断点续传详情请参见开发指南中的[断点续传](/intl.zh-CN/开发指南/对象/文件（Object）/上传文件（Object）/分片上传和断点续传.md)。您还可以通过设置生命周期规则来定时清理不需要的Part，详情请参考[管理碎片](/intl.zh-CN/控制台用户指南/上传、下载和管理文件/管理碎片.md)。
